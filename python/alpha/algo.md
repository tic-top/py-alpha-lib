...
- MEDIAN(input: np.ndarray[float], periods: int): Calculate median over a moving window.  For each position, collects valid (non-NaN) values in the window, sorts them, and returns the middle value (average of two middle values if the count is even).
...
- SELMEAN_BTM(x: np.ndarray[float], y: np.ndarray[float], periods: int, n: int): Mean of x for the bottom-n rows ranked by y over a rolling window of `periods`.  selmean_btm(x, y, d, n): over the past d periods, sort by y ascending, take the bottom n rows, return mean(x) of those rows.
- SELMEAN_DIFF(x: np.ndarray[float], y: np.ndarray[float], periods: int, n: int): Difference between top-n mean and bottom-n mean of x ranked by y.  selmean_diff(x, y, d, n) = selmean_top(x, y, d, n) − selmean_btm(x, y, d, n)
- SELMEAN_TOP(x: np.ndarray[float], y: np.ndarray[float], periods: int, n: int): Mean of x for the top-n rows ranked by y over a rolling window of `periods`.  selmean_top(x, y, d, n): over the past d periods, sort by y ascending, take the top n rows, return mean(x) of those rows.
